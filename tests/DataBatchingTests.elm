...
asCsv content =
    List.tail content
        |> Maybe.withDefault []
        |> CsvData (parseCsv content |> .headers)
...
batchSizeTests =
    describe "batch size tests"
        [ test "calculates correct batch size for wide csv data"
            (\_ ->
                Expect.lessThan 1500 <| calculateCsvBatchSize <| asCsv wideCsvData
            )
        , test "calculates correct batch size for narrow csv data"
            (\_ ->
                Expect.greaterThan 30000 <| calculateCsvBatchSize <| asCsv narrowCsvData
            )
